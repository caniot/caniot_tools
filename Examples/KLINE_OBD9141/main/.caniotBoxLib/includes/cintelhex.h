...
typedef struct  {
	uint32_t        ihrs_minAddress;   //!< minAddress.
	uint32_t        ihrs_maxAddress;   //!< maxAddress.
	ihex_rdata_t 		ihrs_data_set; //!< A list of record (with ihrs_count elements).
} ihex_dataset_t;
...
// METHOD DECLARATIONS
int ihex_get_data(ihex_recordset_t *rs,uint8_t*buffer, uint32_t min,uint32_t max);
...
ulong_t ihex_rs_get_data_size(ihex_recordset_t* rs);
...
int ihex_rs_get_all_address_range(ihex_recordset_t *rs, uint32_t *min, uint32_t *max);
...
bool ihex_extract_data(ihex_recordset_t *appl_hex_record,uint8_t * Buffer,uint32_t dataReadLength);
