...
1. open  `ESP-IDF 4.4.4 CMD`, Go to the project directory with `cd {YOUR PATH}\caniot_tools\Examples\RAW_BYTES_PROTOCOL`  .
...
5. browse to  `/build_s1` or `/build_s3` for HW V3.0 folder and select `RAW_BYTES_PROTOCOL.bin`.
6. alternatively you can copy  `RAW_BYTES_PROTOCOL.bin` to `caniotbox_images`  directory in the sd card and flash it from the `utility->CaniotBox Update:`  menu in CaniotBox.
