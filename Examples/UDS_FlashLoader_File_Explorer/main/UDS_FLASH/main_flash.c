...
{
    appl_message_t ConfigFile = {0};
    ConfigFile.length = 0;
    ConfigFile.bufferPtr = fat12_ReadConfigFile(&ConfigFile.length);
    if (ConfigFile.bufferPtr != NULL && ConfigFile.length > 0)
    {
        cJSON *configFilejson = cJSON_Parse(ConfigFile.bufferPtr);
        free(ConfigFile.bufferPtr);
        if (configFilejson != NULL)
        {
            char *FileNameToBeUse = cJSON_GetObjectItem(configFilejson, "FileNameToBeUse")->valuestring;
            if (FileNameToBeUse != NULL)
            {
                appl_messagefile[appl_messagefile_size].length = 0;
                appl_messagefile[appl_messagefile_size].bufferPtr = fat12_ReadFile(FileNameToBeUse, &appl_messagefile[appl_messagefile_size].length);
                if (appl_messagefile[appl_messagefile_size].length > 0 && appl_messagefile[appl_messagefile_size].bufferPtr != NULL)
                {
                    appl_write_file_sync("/sdcard/newfile.bin", appl_messagefile[appl_messagefile_size].bufferPtr, appl_messagefile[appl_messagefile_size].length,true);
                    appl_messagefile_size++; /* application bin is available, no need to read from sdcard*/
                }
                else
                {
                    appl_messagefile[appl_messagefile_size].bufferPtr = fat12_ReadFile(NULL, &appl_messagefile[appl_messagefile_size].length);
                    if (appl_messagefile[appl_messagefile_size].length > 0 && appl_messagefile[appl_messagefile_size].bufferPtr != NULL)
                    {
                        appl_write_file_sync("/sdcard/newfile.bin", appl_messagefile[appl_messagefile_size].bufferPtr, appl_messagefile[appl_messagefile_size].length,true);
                        appl_messagefile_size++; /* application bin is available, no need to read from sdcard*/
