...
extern void fat12Task(void *TaskArg);
...
void app_main(void)
{
  ...
  xTaskCreate(fat12Task, "fat12Task", 3020, (void *)0, 15, NULL);
  uint8_t nvs_connection_var;
  nvsIf_get_connection_var(&nvs_connection_var);
  if (nvs_connection_var == 1)
  {
    tinyusb_config_t tusb_cfg = {}; // the configuration using default values
    memset(&tusb_cfg, 0, sizeof(tinyusb_config_t));
