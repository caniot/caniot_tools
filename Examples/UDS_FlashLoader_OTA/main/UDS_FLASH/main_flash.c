...
extern char *uds_http_ota_task(const char *filePath, uint32_t *fileLength);
...
static void getFileDataFlash(const cJSON *const mainArray)
{
    ...
    else /*download from your server*/
    {
        ...
        {
            appl_messagefile[appl_messagefile_size].bufferPtr = uds_http_ota_task(fileName, &appl_messagefile[appl_messagefile_size].length);
            if (appl_messagefile[appl_messagefile_size].bufferPtr != NULL && appl_messagefile[appl_messagefile_size].length > 0)
                appl_messagefile_size++;
        }
    }
    if (min != NULL)
