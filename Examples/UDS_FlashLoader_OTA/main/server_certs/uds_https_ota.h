...
typedef void *uds_https_ota_handle_t;
 ...
 *           then no other APIs from `uds_https_ota` component should be called.
 *           If more information and control is needed during the HTTPS OTA process,
 *           then one can use `uds_https_ota_begin` and subsequent APIs. If this API returns
 ...
 */
esp_err_t uds_https_ota(const esp_http_client_config_t *config);

/**
 * @brief    Start HTTPS OTA Firmware upgrade
 *
 * This function initializes ESP HTTPS OTA context and establishes HTTPS connection.
 * This function must be invoked first. If this function returns successfully, then `uds_https_ota_perform` should be
 * called to continue with the OTA process and there should be a call to `uds_https_ota_finish` on
 ...
 *
 * @param[in]   ota_config       pointer to uds_https_ota_config_t structure
 * @param[out]  handle           pointer to an allocated data of type `uds_https_ota_handle_t`
 ...
 * must be called only if uds_https_ota_begin() returns successfully.
 ...
 *
 * @param[in]  https_ota_handle  pointer to uds_https_ota_handle_t structure
 *
 * @return
 ...
 *
 * @note    This API can be called just before uds_https_ota_finish() to validate if the complete image was indeed received.
 *
 * @param[in]   https_ota_handle pointer to uds_https_ota_handle_t structure
 *
 * @return
 ...
 *           uds_https_ota_finish should not be called after calling uds_https_ota_abort
 *
 * @param[in]  https_ota_handle   pointer to uds_https_ota_handle_t structure
 *
 * @return
 ...
 * This function closes the HTTP connection and frees the ESP HTTPS OTA context.
 *
 * @note     uds_https_ota_abort should not be called after calling uds_https_ota_finish
 *
 * @param[in]  https_ota_handle   pointer to uds_https_ota_handle_t structure
 *
 * @return
 ...
 *
 * @note    This API can be called only after uds_https_ota_begin() and before uds_https_ota_perform().
 ...
 */
esp_err_t uds_https_ota_get_img_desc(uds_https_ota_handle_t https_ota_handle, esp_app_desc_t *new_app_info);


/**
* @brief  This function returns OTA image data read so far.
*
* @note   This API should be called only if `uds_https_ota_perform()` has been called atleast once or
*         if `uds_https_ota_get_img_desc` has been called before.
*
* @param[in]   https_ota_handle   pointer to uds_https_ota_handle_t structure
*
* @return
...
*
* @note   This API should be called after uds_https_ota_begin() has been already called.
*         This can be used to create some sort of progress indication
*         (in combination with uds_https_ota_get_image_len_read())
*
* @param[in]   https_ota_handle   pointer to uds_https_ota_handle_t structure
*
* @return
