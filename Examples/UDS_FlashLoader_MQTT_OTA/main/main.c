...
#ifdef MQTT_VARIANT_PUBLISCHER
extern void lvgl_create_publish_container(lv_obj_t *parent);
extern void mqtt_hiveMQ_publisher_init(void);
#else
extern void mqtt_hiveMQ_subscriber_init(void);
#endif
...
void app_main(void)
{
  ...
  {
#ifdef MQTT_VARIANT_PUBLISCHER
    lvgl_create_publish_container(main_table);
    mqtt_hiveMQ_publisher_init();
#else
    mqtt_hiveMQ_subscriber_init();
#endif
  }
}
